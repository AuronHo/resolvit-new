...
class ViewDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final serviceName = service['NamaJasa']?.toString() ?? 'Service Detail';
    final kategori = service['Kategori']?.toString() ?? '-';
    final rating =
        ((service['RatingRataRata'] as num?) ?? 0.0).toStringAsFixed(1);
    final price = service['HargaMulai'] != null
        ? 'Start from Rp ${service['HargaMulai']}'
        : 'Contact us';
    final desc = service['DeskripsiJasa']?.toString() ?? '-';
    final avatarUrl = service['ImageUrl']?.toString() ?? '';
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            _buildInfoRow('Rating', '$rating / 5.0'),
            _buildInfoRow('Price Range', price),
            _buildInfoRow('Description', desc),
            _buildHoursRow(hoursJson),
            if (location.isNotEmpty) _buildLocationRow(context, location),
  ...
  Widget _buildInfoRow(String label, String value) {
    return _buildCustomRow(label, Text(value,
        style: const TextStyle(color: Colors.black87, fontSize: 13)));
  }

  Widget _buildCustomRow(String label, Widget valueWidget,
                ...
                return Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(
                        isOpen ? '${d['from']} - ${d['to']}' : 'Closed',
                        style: TextStyle(
                          fontSize: 12,
                          color: isOpen ? Colors.black87 : Colors.red,
                          fontWeight:
                              isOpen ? FontWeight.normal : FontWeight.bold,
                        ),
                      ),
                    ],
                  ),
